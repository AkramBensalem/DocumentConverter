package me.akram.bensalem.documentconverter.service
...
import me.akram.bensalem.documentconverter.data.OcrResult
import me.akram.bensalem.documentconverter.data.Options
import me.akram.bensalem.documentconverter.data.TestResult
import me.akram.bensalem.documentconverter.data.request.OcrRequest
import me.akram.bensalem.documentconverter.data.response.OcrResponse
import me.akram.bensalem.documentconverter.data.response.SignedUrlResponse
...
class DocumentConverterService {
    private val log = Logger.getInstance(DocumentConverterService::class.java)
    ...
    suspend fun uploadFile(document: Path, apiKey: String): UploadResponse {
    ...
    /**
     * Converts document to Markdown using external MarkItDown CLI (offline mode)
     */
    private fun runOfflineConversion(document: Path, targetDir: Path, options: Options): OcrResult {
        try {
            log.info("Starting offline conversion via MarkItDown CLI for: $document")
            ...
            return OcrResult(
                ...
                imageFiles = emptyList(),
                createdFiles = emptyList(),
                error = "MarkItDown CLI not found. Please install it and ensure it's available on PATH, or set its full path in Settings (Tools → Document Converter). Details: ${e.message}"
            )
        } catch (e: Exception) {
    ...
    suspend fun convertDocument(
        ...
        return try {
            ...
            log.warn("Document Converter failed for $document", e)
            OcrResult(
                markdownFile = null,
                jsonFile = null,
                imageFiles = emptyList(),
    ...
    private suspend fun runMistralOCR(document: Path, options: Options): OcrResponse {
        if (options.apiKey.isBlank()) throw Exception("API key is missing. Set it in Settings/Preferences → Tools → Document Converter.")
        return try {
    ...
    suspend fun testConnection(apiKey: String): TestResult {
        if (apiKey.isBlank()) return TestResult(
            false,
            "API key is missing. Set it in Settings/Preferences → Tools → Document Converter."
        )
        return try {
    ...
    companion object {
        @JvmStatic
        fun getInstance(project: Project): DocumentConverterService = project.service()
